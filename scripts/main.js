...
const petBuilder = (petArray) => {
  ...
  petArray.forEach((p, i) => {
    domString += `<div class="pet-card" style="width: 18rem;" id=${i}>
      <h5 class="card-title bg-warning text-center">${p.name}</h5>
...
const deleteCard = (e) => {
  ...
  if (targetType === "button") {
    // const deletePet = selectedPets.findIndex((selectedPets) => selectedPets.id === targetId);
...
const buttonEvents = () => {
  ...
  document.querySelector('#pets').addEventListener('click', deleteCard);
};
